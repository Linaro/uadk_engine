...
#define UADK_SECURITY_CHECK_DISABLE	0
...
static int ecdh_bit_check(const EC_GROUP *group)
{
	...
	fprintf(stderr, "invalid: unsupport key bits %d for keyexch!\n", bits);
...
static int ecdh_check_key(OSSL_LIB_CTX *ctx, const EC_KEY *ec, int protect)
{
...
static int uadk_keyexch_ecdh_init(void *vpecdhctx, void *vecdh, const OSSL_PARAM params[])
{
	...
	return ecdh_check_key(pecdhctx->libctx, vecdh, 1);
}
...
static int uadk_keyexch_ecdh_set_peer(void *vpecdhctx, void *vecdh)
{
	...
	ret = ecdh_check_key(pecdhctx->libctx, vecdh, 1);
	if (!ret)
		return ret;

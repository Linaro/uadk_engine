...
BIGNUM *e_value, BN_CTX *ctx, BN_GENCB *cb)
{
		...
		if (ERR_GET_LIB(error) == ERR_LIB_BN && ERR_GET_REASON(error) == BN_R_NO_INVERSE)
				...
				goto err;
		if (!BN_GENCB_call(cb, 2, *n++))// When the n-th is rejected, call
		//BN_GENCB_call(cb, 2, n)
				goto err;
		return 0;
err:
		return -1;
...
redo:
		//if (hpre_get_prime_once(i, bitsr, &n, prime, rsa_p, rsa_q, r1, r2,
		//e_value, ctx, cb) == KAE_FAIL))
				//goto err;
		bitse += bitsr[i];
		//FIXME
		int ret = 1;
		//int ret = check_prime_sufficient(&i, bitsr, &bitse, &n, rsa_p,
		//rsa_q, r1, r2, ctx, cb);
		if (ret == -1) {
				goto err;
...
static uadk_rsa_sess_t *uadk_get_eng_session(RSA *rsa, int bits, int is_crt)
{
		...
		rsa_sess = uadk_new_eng_session(rsa);
		if (rsa_sess == NULL) {
...
static int uadk_rsa_keygen(RSA *rsa, int bits, BIGNUM *e, BN_GENCB *cb)
{
		...
		ret = uadk_rsa_fill_keygen_data(rsa_sess->sess,
		&rsa_sess->req, wd_e, wd_p, wd_q);
		if (!ret) {
				  ...
				  unsigned char *to, RSA *rsa, int padding)
{
		...
		RSA_get0_key(rsa, &n, &e, &d);
		//hpre_rsa_check(flen,n,e,&num_bytes,rsa);
		key_bits = RSA_bits(rsa);
		rsa_sess = uadk_get_eng_session(rsa, 0, 1);
		num_bytes = BN_num_bytes(n);
		in_buf = (unsigned char *)OPENSSL_malloc(num_bytes);
		...
		BN_bin2bn((const unsigned char *)rsa_sess->req.dst,
		rsa_sess->req.dst_bytes, bn_ret);
		bn_get_words(bn_ret)[0];
		//if ((padding == RSA_X931_PADDING) && ((bn_get_words(bn_ret)[0] & 0xf) != 12))
		//{  not 12 then BN_sub
		//}
		BN_bn2binpad(bn_ret, in_buf, num_bytes);
